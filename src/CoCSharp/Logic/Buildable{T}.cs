...
namespace CoCSharp.Logic
{
    ...
    public abstract class Buildable<TCsvData> : VillageObject<TCsvData> where TCsvData : CsvData, new()
    {
        ...
        internal Buildable(Village village, CsvDataRowRef<TCsvData> data, int level) : base(village, data)
        {
            Initialize(level);
        }

        internal Buildable(Village village, CsvDataRowRef<TCsvData> data, object userToken, int level) : base(village, data)
        {
            Initialize(level);
            UserToken = userToken;
        }

        internal Buildable(Village village, CsvDataRowRef<TCsvData> data, int x, int y, int level) : base(village, data, x, y)
        {
            Initialize(level);
        }

        internal Buildable(Village village, CsvDataRowRef<TCsvData> data, int x, int y, object userToken, int level) : base(village, data, x, y)
        {
            Initialize(level);
            UserToken = userToken;
        }
